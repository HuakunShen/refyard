<!--
  Appearance and theme settings modal.
  Provides controls for Light/Dark mode, Accent colors, custom background wallpaper,
  and frosted glassmorphism effect.
-->
<script lang="ts">
  import {
    Dialog,
  ...
  <DialogTrigger>
    {#snippet child({ props })}
      <Button
        ...
        size="sm"
        variant="ghost"
        class={cn("gap-1.5 text-xs text-ink-muted hover:text-ink", className)}
  ...
  >
    <DialogHeader>
      ...
      <DialogDescription class="text-xs text-ink-muted">
        Customize accent colors, workbench background, and glassmorphism
        styling.
      </DialogDescription>
    </DialogHeader>
        ...
        <div class="grid grid-cols-3 gap-2">
          {#each PRESET_BACKGROUNDS as preset (preset.id)}
            ...
            >
              ...
              <span
                class="text-[10px] text-ink-faint leading-tight line-clamp-1"
              >
                {preset.desc}
              </span>
            </button>
    ...
    <DialogFooter class="sm:justify-between">
      <span class="text-[11px] text-ink-faint">Preferences saved locally</span>
