...
| `pnpm test:unit`        | unit suites (contract, core planners/parsers, graph, client, ui, fixtures) — 233 cases, 15 files                                                                                                                                                        | verified |
| `pnpm test:integration` | real Git in temporary repositories: reads, writes, merge, worktrees, submodules, jobs, restart, auth, concurrency, network, repository creation, the four repository shapes; plus the negative security cases in `tests/security` — 345 cases, 22 files | verified |
...
passed** — `check`, `check:boundaries`, `check:contract`, `test:unit` (233), `test:integration`
(344), `test:pack`, `test:portable`, `build:release`, `pack:smoke` (14 steps), `bench:runtime`
