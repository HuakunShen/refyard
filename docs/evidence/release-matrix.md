...
## Historical R15 API-only release evidence

At `0c3cd6d` on 2026-09-16, the API-only `refyard@0.1.2` artifact passed
`pnpm test:pack` (17/17), `pnpm pack:smoke` (14 steps), full three-engine E2E
(105/105), compatibility (9/9), and the runtime benchmark. The Wrangler 4.132.0
dry-run read 87 assets without uploading them. These results predate the restored
local workbench and are not verification of the merged revision. The newer complete
benchmark receipt in [performance.json](performance.json) identifies its own build
and measurement timestamp; the earlier receipt remains in Git history at `701e183`.
The npm publication workflow was locally tested; no publication or live deployment
