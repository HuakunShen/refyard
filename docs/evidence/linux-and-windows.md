...
deliberately *not* used as that check — on Windows it reports three files as modified because the
...
1. **A deleted-and-recreated repository kept its approval on Linux.**
   ...
   on ext4 `rm -r root && mkdir root` hands the *same* inode number back — measured on that
   ...
   filesystem and silently survives it on another is not a check. Directories are now *pinned* with
...
- The e2e repository-creation spec asserted `main` for a repository created with an *empty* branch
  ...
  does and Git said `master`. The spec fills the field now. What the default *is* stays covered in
...
- **CI itself**: `.github/workflows/ci.yml` has still not run. The push that triggers it is the
  first run, and this file will say so until a run is recorded in `release-matrix.md`.
