...
Workers Static Assets in the supported deployment shape); the CLI package never hosts it. It is
not published to a registry by this repository: you build a tarball and install that.
...
Build and dry-run the static PWA from `apps/web`:

```sh
pnpm build
pnpm --dir apps/web exec wrangler deploy --dry-run
# Set PUBLIC_API_ORIGINS in apps/web/wrangler.jsonc to the exact HTTPS tunnel origin first.
