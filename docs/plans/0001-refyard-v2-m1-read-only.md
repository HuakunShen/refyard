...
`packages/git-core/src/{ports.ts,bytes/*,parse/*,plan/*}`, `tests/core/{formats,plans}.test.ts`.
Parsers: status (porcelain v2 `-z`, renames with two NUL paths), worktree list, refs, numstat,
name-status, `cat-file --batch` framing, patch grammar, push/fetch porcelain. Planners return
`GitCommandSpec { argv, stdin?, cwdHandle }` — never a shell string; `stagePaths` uses
`--literal-pathspecs` with NUL stdin; `commit` passes the message via `-F -`.
