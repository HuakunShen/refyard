...
> Status: **active, revision 3** — written 2026-09-16, after 0.1.1 was published; revised after
...
Published baseline: **refyard 0.1.1** (historically `UNLICENSED`, `engines: >=22 <27`), verified
against the registry. The current release extension prepares `0.1.2` as `AGPL-3.0-only`.
...
The current release extension prepares **refyard 0.1.2** as an `AGPL-3.0-only` package, adds a
tag-triggered npm Trusted Publishing workflow, and exposes the official Cloudflare self-deploy
button for the static Worker. The local release gates now pass on the current macOS checkout;
public-repository visibility, the GitHub Actions publish run, npm registry publication, and a
current-head live Worker deployment remain external evidence rather than assumptions.
...
6. **R10–R11** are T16 and T17 (Xross, Kunkun). Both need the _current_ external APIs read from
...
**The loop, per task.** Write the failing case first and _prove it fails for the right reason_
...
- **The root TypeScript program has no DOM lib.** `tsconfig.json` covers `scripts/` and `tests/`
  ...
  will not type-check there. Adding `"DOM"` to that program is _not_ the fix: it also replaces
...
| ---- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| R1   | the token and the instance id leave `localStorage`; a legacy copy is deleted on read                              | `pnpm check`, `vitest tests/unit/web-storage.test.ts`, the new e2e case in chromium                                                                                                                                                                                                                                    | **7 unit cases and the browser case pass; both fail against the old behaviour.** Commit `e78fbc7`. The full three-engine `pnpm test:e2e` did _not_ run on that revision — it was interrupted by the handover. Run it before trusting the row                                                                                    | the case comments; `storage-policy.ts`                                                                                                          |
...
| R5   | real SPA/host compatibility suite; additive fields tolerated, major skew refused, newer same-major writes blocked | `pnpm check`; `pnpm test:compat`                                                                                                                                                                                                                                                                                       | **9/9 passed across Chromium, Firefox and WebKit against the built SPA, separate static host and real API-only CLI; WebKit response rewrites isolate service workers and CORS preflights, and the unit negotiation case passes.**                                                                                               | `tests/compat/compat.spec.ts`; `tests/compat/playwright.config.ts`; `tests/unit/session-negotiation.test.ts`; `docs/evidence/release-matrix.md` |
...
| R8   | complete                                                                                                          | `pnpm check`; `pnpm test:integration`                                                                                                                                                                                                                                                                                  | **Hono API adapter, OpenAPI/Scalar discovery and read-only MCP are wired; 4 Hono/MCP cases and all 383 Node/integration/security cases pass.**                                                                                                                                                                                  | `packages/host-node/src/http/hono-app.ts`; `tests/integration/hono.test.ts`; `docs/evidence/release-matrix.md`                                  |
...
| R15  | Worker static PWA, API-only CLI, exact hosted API origin                                                          | `pnpm check`; `pnpm test:web-host`; `wrangler deploy --dry-run`; `pnpm test:e2e`; `pnpm test:compat`; `pnpm pack:smoke`                                                                                                                                                                                                | **Implemented locally. Check 8/8; unit 262; integration 383; Worker dry-run read 87 files; Worker tests 4/4; full e2e 105/105 across Chromium, Firefox and WebKit; compatibility 9/9; package smoke 14 steps. No current-head live deployment or public tunnel was used.**                                                      | `docs/evidence/release-matrix.md`; `docs/evidence/security.md`                                                                                  |

## R1 — The session token leaves `localStorage`

**What:** `apps/web/src/lib/storage.ts` writes the bearer to `sessionStorage` _and_ `localStorage`
...
`localStorage` goes; the read keeps a fallback so a token stored by an earlier build is _cleared_
rather than used.
...
that a third repository on the same disk is still refused; a case asserts the _silent_ last-wins is
...
registration request carries a path the _user_ chose, and the host checks containment and
...
(reads both understand keep working; writes are blocked), and a _service_ field the page does not
...
refusal _and_ that nothing was written; no test may weaken the refusal to make an older page work.
...
_preserved and run_ rather than bypassed, and limit abuse (an oversized `pathIds` set, an oversized
...
**Rules:** every case asserts the _effect on the repository_, not only the status code; a case that
...
4. a hook the user has installed must **run** (the product never adds `--no-verify`): install a
   pre-commit hook that fails, attempt a commit, assert the commit was refused _by the hook_ and
...
a _derivation_ rather than a second description.
...
| An app definition to declare "refyard, version X, needs Node and Git" | **Does not exist.** There is no app registry or launcher; `ServiceRegistry` registers typed _services_, not programs. The only "remote apps" text is CrossCopy's legacy inventory, marked design-only and non-normative.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
and its `egress-allow` entry for the port. Both are the _peer's_ decisions; the integration presents
...
## Public release extension — AGPLv3, npm Trusted Publishing, and self-deploy

This release extension carries the owner's distribution direction without changing the local-first
trust boundary. The root package is the workspace-only build coordinator; `packages/npm-dist` is
the publishable API-only package; `apps/web` is the independently deployable static PWA.

| Surface                | Local result                                                                                                                                                                                                                                              |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| License and identity   | Root and publishable manifests use `AGPL-3.0-only`; `LICENSE` contains the complete GNU AGPL v3 notice; package version is `0.1.2` with the GitHub repository metadata.                                                                                   |
| npm publisher          | `.github/workflows/publish.yml` triggers only on `v*`, grants `id-token: write`, checks the tag against the package version, and runs `npm publish` from `packages/npm-dist` without `NPM_TOKEN` or `NODE_AUTH_TOKEN`; the workflow contract test passes. |
| Cloudflare self-deploy | Root `wrangler.jsonc` and `pnpm deploy` target an asset-only Worker; the official Deploy button is in `README.md`; the current dry-run passed with Wrangler 4.132.0 and 87 assets, with no upload.                                                        |
| Release gates          | `check`, boundaries, contract, unit (262), integration (383), pack (17), portable (4), e2e (105/105), compatibility (9/9), package smoke (14 steps), and benchmark all passed on the macOS release machine; the rebuilt artifact is `refyard@0.1.2`.      |
| External status        | Public-repository visibility, the release tag, GitHub Actions publish run, npm registry update/provenance, and current-head live Worker deployment remain unverified until the owner performs and verifies those external actions.                        |

The untracked `docs/product/git-client-direction.md` file was present during the benchmark and is
preserved as owner work; the performance report records the resulting dirty-worktree fact. It is
not part of this release extension's staged files.
